...
public class DoubleCritter extends Critter
{
	private int lift = 120;
	private int defaultAdultPeriod = 100;
	private String DNA = "";
	private String lover_DNA = "";
	// a DoubleCritter needs time to grow up
	private boolean ifadult = false;
	private boolean ifready = false;
	private boolean ifmale;
	private boolean ifloving = false;
	private boolean ifnear = false;
	private boolean iffacing = false;
	private boolean iffocus = false;
	// ifbirthing is true limited to female
	private boolean ifbirthing = false; 
	private DoubleCritter lover = null;
	private int rate = 8;
	private int defaultPeriodReady = 10;
	private int defaultBirthRate = 80;
	private int defaultBirthRateDecrease = 15;
	private int[] currentColor = {0, 0, 0};
	private Color loverColor = null;
	
	public DoubleCritter(String dna, Color col, int option)
	{ // DNA must be setup when creating a new DoubleCritter
	  // 0-male, 1-female, other-ramdon
	...
	public void setLover(DoubleCritter lov)
	{ // set its belove
		this.lover = lov;
	}
	...
	private void giveBirth()
	{
		...
		DoubleCritter child = new DoubleCritter(
				DoubleCritter.mixDNA(this.lover_DNA, this.getDNA()),
				DoubleCritter.mixColor(this.getColor(), this.loverColor, bol), 7
				);
	 ...
	 */
	public static void letsDancing(DoubleCritter a)
	{
	...
	public void processActors(ArrayList<Actor> actors)
    {
        ...
        { // lift is running away
        	...
        	for (Actor a : actors)
            {
        		if ((a instanceof DoubleCritter)) {
        			if (this.ifready && ((DoubleCritter)a).getGender() != this.getGender() && ((DoubleCritter)a).isReady())
        			{ // has found out its lover and then change their own statements
        				this.setReady(false);
            			this.setLover((DoubleCritter)a);
            			this.setLoving(true);
            			((DoubleCritter)a).setReady(false);
            			((DoubleCritter)a).setLover(this);
            			((DoubleCritter)a).setLoving(true);
            			break;
        			}
        			if (this.ifmale && this.ifadult && !((DoubleCritter)a).ifadult && ((DoubleCritter)a).ifmale)
        			{ // not its son
        				if (((DoubleCritter)a).getDNA().indexOf(DoubleCritter.mixDNA(this.DNA, "")) == -1) 
	...
	public void makeMove(Location loc)
    {
		...
		else if (this.isFocus())
		{
			if (this.lover.isFocus())
			{
				if (this.rate != 0)
				{
					DoubleCritter.letsDancing(this);
					this.rate--;
				}
				else
				{
