...
public class QImageView extends ImageView {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (width > mIns.getScreenWidth()) {
			// 如果图宽大于屏宽，就不用水平居中
			center(false, true);
		} else if(height >(mIns.getAlbumHeight())){
			center(true, false);
		}else{
			center(true, true);
		}
	...
	private void arithScaleRate() {
		float scaleWidth = GlobalApplication.getInstance().getScreenWidth() / (float) imageWidth;
		float scaleHeight = GlobalApplication.getInstance().getAlbumHeight()/ (float) imageHeight;
	...
	public void setImageBitmap(Bitmap bitmap) {
		if (BuildConfig.DEBUG) Log.d("QImageView", "Entering setImageBitmap");
		...
		arithScaleRate();
		//缩放到屏幕大小
		zoomTo(scaleRate,GlobalApplication.getInstance().getScreenWidth() / 2f, GlobalApplication.getInstance().getAlbumHeight() / 2f);

		//居中
		layoutToCenter();

		if (BuildConfig.DEBUG) Log.d("QImageView", "Leaving setImageBitmap");
	}
	...
	public void layoutToCenter()
	{
		...
		//空白区域宽高
		float fill_width = GlobalApplication.getInstance().getScreenWidth()- width;
		float fill_height = GlobalApplication.getInstance().getAlbumHeight() - height;
	...
	protected float getValue(Matrix matrix, int whichValue) {
		...
		mMinZoom =( GlobalApplication.getInstance().getScreenWidth()/2f)/imageWidth;
