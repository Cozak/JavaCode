...
function drawAllTransactionsTable(){
	var data = [
...
function drawJVMCharts(heap, gcRate, cpuTime, threadCount){
	var heap = [{'label':"11:00", 'value':[55,79]}, {'label':"11:05", 'value':[65,78]},{'label':"11:10", 'value':[45,89]}, {'label':"11:15", 'value':[35,98]}]
	...
	gcData.datasets[0].title = "GC Rate(count/second)";
	ctx = $("#jvmGCChart").get(0).getContext("2d");
	new Chart(ctx).Line(gcData, {datasetFill:true,pointDot:false});
	...
	new Chart(ctx).Line(threadData, {datasetFill:true,pointDot:false});
...
function initTimeRange(){
	...
	var formatDateTime = function(d){
		return d.getFullYear()+"-"+toTwoDig(d.getMonth())+"-"+toTwoDig(d.getDate())+"T"+toTwoDig(d.getHours())+":"+toTwoDig(d.getMinutes());
	}
	var now = new Date();
	...
	$('#changeTimeRangeBtn').bind('click', function(){
		alert('timeRangeStart:'+$('#timeRangeStart').val()+"~"+$('#timeRangeEnd').val());
	});
}
$(document).ready(function(){
	drawRequestStatChart();
